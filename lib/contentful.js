...
class ContentfulService {
  ...
  async getProvidersByFilter(filters) {
    const entries = await contentClient.getEntries({
      ...
      ...(filters.recruitCost?.length && {
        'fields.recruitCost[in]': filters.recruitCost.join(','),
      }),
      ...(filters.followUpCost?.length && {
        'fields.followUpCost[in]': filters.followUpCost.join(','),
      }),
  ...
  async getProviderFilterOptionValues() {
    ...
    return {
      ...
      findCost: providerContentType.fields.find((f) => f.id === 'findCost')
        .validations[0].in,
      recruitCost: providerContentType.fields.find(
        (f) => f.id === 'recruitCost'
      ).validations[0].in,
      followUpCost: providerContentType.fields.find(
        (f) => f.id === 'followUpCost'
      ).validations[0].in,
    };
  }
