...
class ContentfulService {
  ...
  async getProviderFilterOptionValues() {
    ...
    return {
      serviceTypes: providerContentType.fields.find(
        (f) => f.id === 'serviceType'
      ).items.validations[0].in,
      dataTypes: providerContentType.fields.find((f) => f.id === 'dataTypes')
        .items.validations[0].in,
      geographies: providerContentType.fields.find((f) => f.id === 'geography')
        .items.validations[0].in,
      providerOrganisations: providerOrganisations.items,
    };
  }
