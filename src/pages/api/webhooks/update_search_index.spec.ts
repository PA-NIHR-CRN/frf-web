...
test('Updates search index when a DSP is published', async () => {
  ...
  const res = await testHandler(handler, { method: 'GET', body: { id: 'test', contentType: 'serviceProvider' } })
...
test('Updates search index when an embedded content type is published', async () => {
  ...
  const res = await testHandler(handler, { method: 'GET', body: { id: 'test', contentType: 'serviceTypeBlock' } })
