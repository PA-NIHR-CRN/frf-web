...
test('Allows non-JavaScript users to apply filters', () => {
  ...
  expect(screen.getByRole('button', { name: 'Apply filters - search criteria' })).toBeInTheDocument()
})
...
test('Allows clearing all filters', () => {
  render(<Filters {...defaultProps} />)
  expect(screen.getByRole('link', { name: 'Clear all filters - search criteria' })).toHaveAttribute(
    'href',
    '/providers'
  )
})
